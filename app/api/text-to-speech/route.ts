...
export async function POST(request: Request) {
  try {
    ...
    if (name && voiceCache[name]) {
      console.log(`[TTS] Using cached voice ID for ${name}: ${voiceCache[name]}`);
    ...
    // If profile is provided but no voiceId, try to generate through voice-design API
      ...
      console.log(`[TTS] No cached voice found for ${profile.name}, generating new voice...`);
      ...
      try {
        ...
        if (voiceResponse.ok) {
          ...
          console.error(`[TTS] Voice design API error: ${voiceResponse.status}`);
          voiceId = process.env.DEFAULT_VOICE_ID || 'pNInz6obpgDQGcFmaJgB';
        }
      } catch (error) {
        console.error('[TTS] Error calling voice-design API:', error);
        voiceId = process.env.DEFAULT_VOICE_ID || 'pNInz6obpgDQGcFmaJgB';
      }
    }
      ...
      if (errorData.detail?.status === 'voice_not_found' && name) {
        ...
        try {
          ...
          if (regenerateResponse.ok) {
            ...
            const regeneratedVoiceId = regenerateData.voiceId;
            
            // Try again with the regenerated voice
            ...
            if (retryResponse.ok) {
              ...
              console.error('[TTS] Retry with regenerated voice failed');
            }
          }
        } catch (regenerateError) {
          console.error('[TTS] Error regenerating voice:', regenerateError);
        }
        
        // If regeneration failed or was not attempted, fall back to default voice
        console.log('[TTS] Falling back to default voice');
